...
existing rubric prompt to an OpenAI-compatible chat completions endpoint, and
writes score_images.py-compatible JSON on stdout.
"""
...
def redact_secret(text: str) -> str:
    ...
    return re.sub(r"ark-[A-Za-z0-9_-]{8,}", "ark-REDACTED", text)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--base-url", default=os.environ.get("PANGU_BASE_URL", DEFAULT_BASE_URL))
    parser.add_argument("--model", default=os.environ.get("PANGU_JUDGE_MODEL", DEFAULT_MODEL))
    parser.add_argument("--prompt-version", default=os.environ.get("PANGU_JUDGE_PROMPT_VERSION", "aesthetic-v4"))
    parser.add_argument(
        "--output-mode",
        choices=["full", "score-only"],
        default=os.environ.get("PANGU_JUDGE_OUTPUT_MODE", "full"),
    )
    parser.add_argument("--api-key-env", default="PANGU_API_KEY")
    parser.add_argument("--temperature", type=float, default=float(os.environ.get("PANGU_JUDGE_TEMPERATURE", "0")))
    parser.add_argument("--max-tokens", type=int, default=int(os.environ.get("PANGU_JUDGE_MAX_TOKENS", "1200")))
    parser.add_argument("--timeout", type=float, default=float(os.environ.get("PANGU_JUDGE_TIMEOUT", "240")))
    parser.add_argument("--max-retries", type=int, default=int(os.environ.get("PANGU_JUDGE_MAX_RETRIES", "4")))
    parser.add_argument("--parse-max-retries", type=int, default=int(os.environ.get("PANGU_JUDGE_PARSE_MAX_RETRIES", "2")))
...
def main() -> int:
    ...
    try:
        ...
        for parse_attempt in range(args.parse_max_retries + 1):
            raw, api_retry_count = call_model(args, request, token, parse_retry=parse_attempt > 0)
